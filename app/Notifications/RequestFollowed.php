...
class RequestFollowed extends Notification implements ShouldQueue
{
    use Queueable;
    public $user;
    public $fcmtoken;
    ...
    {
        return [
            'title' => "Follow up request from {$this->user->username}",
            'type' => 'request_followed',
            'user' => [
                'id' => $this->user->id,
    ...
    public function toFirebase($notifiable)
    {
        return (new FirebaseMessage())
            ->withTitle("Follow up request from {$this->user->username}")
            ->withSound('default')
            ->withPriority('high')
